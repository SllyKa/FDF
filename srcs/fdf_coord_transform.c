...
/*   Updated: 2019/11/12 04:24:33 by gbrandon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			fdf_map_trnsf(t_bilist *map, t_transfrm *tr, void (*f_tr)(t_pointz *, t_transfrm *))
{
	t_pix_lst	*pxl;
...
void	iso_map(t_pointz *p0, t_transfrm *tr)
{
	...
	p0->y = -(p0->z - tr->start->z) + (oldx + oldy) * sin(tr->angle) + tr->start->y;
}